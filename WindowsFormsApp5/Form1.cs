...
namespace WindowsFormsApp5
{
    public partial class Form1 : Form
    {
        ...
        int RayProgramID;
        int RayVertexShader;
        int RayFragmentShader;

        private void InitShaders()
        {
            RayProgramID = GL.CreateProgram();
            ...
            loadShader(vertexShaderText, ShaderType.VertexShader, RayProgramID, out RayVertexShader);
            loadShader(fragmentShaderText, ShaderType.FragmentShader, RayProgramID, out RayFragmentShader);
            GL.LinkProgram(RayProgramID);
            Console.WriteLine(GL.GetProgramInfoLog(RayProgramID));
        ...
        private void Drawing()
        {
            ...
            GL.UseProgram(RayProgramID);
            InitMaterials();
            initSceneBuffers();
            // Camera
            int location = GL.GetUniformLocation(RayProgramID, "uCamera.Position");
            GL.Uniform3(location, new Vector3(0, 0, -7.5f));
            location = GL.GetUniformLocation(RayProgramID, "uCamera.Up");
            GL.Uniform3(location, Vector3.UnitY);
            location = GL.GetUniformLocation(RayProgramID, "uCamera.Side");
            GL.Uniform3(location, Vector3.UnitX);
            location = GL.GetUniformLocation(RayProgramID, "uCamera.View");
            GL.Uniform3(location, Vector3.UnitZ);
            location = GL.GetUniformLocation(RayProgramID, "uCamera.Scale");
            GL.Uniform2(location, new Vector2(1, (float)Glcontrol.Height / Glcontrol.Width));
            // Light
            location = GL.GetUniformLocation(RayProgramID, "uLight.Position");
        ...
        void InitMaterials()
        {
            ...
            {
                location = GL.GetUniformLocation(RayProgramID, "uMaterials[" + i + "].Color");
                GL.Uniform3(location, materials[i].Color);
                location = GL.GetUniformLocation(RayProgramID, "uMaterials[" + i + "].LightCoeffs");
                GL.Uniform4(location, materials[i].LightCoeffs);
                location = GL.GetUniformLocation(RayProgramID, "uMaterials[" + i + "].ReflectionCoef");
                GL.Uniform1(location, materials[i].ReflectionCoef);
                location = GL.GetUniformLocation(RayProgramID, "uMaterials[" + i + "].RefractionCoef");
                GL.Uniform1(location, materials[i].RefractionCoef);
                location = GL.GetUniformLocation(RayProgramID, "uMaterials[" + i + "].MaterialType");
        ...
        private void openGlControl_Paint_1(object sender, PaintEventArgs e)
        {
            Drawing();
        }
    }
